    ...
    def setAlgorithm(self):
        if self.__checkAll():
            ...
            if self.radio_value.get() == 0:
                ...
                self.generator = algo.bubble_sort(A)
            elif self.radio_value.get() == 1:
                self.title = "InsertionSort"
                self.generator = algo.insertion_sort(A)
            elif self.radio_value.get() == 2:
                self.title = "MergeSort"
                self.generator = algo.merge_sort(A, 0, N - 1)
            elif self.radio_value.get() == 3:
                self.title = "QuickSort"
                self.generator = algo.quick_sort(A, 0, N - 1)
            else:
                self.title = "SelectionSort"
                self.generator = algo.selection_sort(A)
